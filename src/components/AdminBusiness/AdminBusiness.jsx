...
const AdminBusiness = () => {
  ...
  return (
    ...
    <h3 className={styles.title}>{t("common.businessManage")}</h3>
     <button className={styles.add} onClick={() => { setEditingBusiness(null); setOpenModal(true); }}>
      ...
      <ul className={styles.ul}>
        {businesses.map((b) => (
          <li className={styles.li} key={b.id} onClick={() => { setEditingBusiness(b); setOpenModal(true); }}>
              <div className={styles.imgCont}>
                {b.photo && <img src={getOptimizedImageUrl(b.photo, 500)} alt={b.name}/>}
              </div>
              <div className={styles.text}>
                ...
                <p className={styles.p}><span>{t("common.categories")}</span>: {b.categories?.join(", ")}</p>
              </div>
